# Raport indywidualny – Sprint 1
Imię i nazwisko: Dominik Piwowarski  
Zespół: - 
Numer sprintu: 1  
Okres: 2025-10-29 – 2025-11-05  

## Zakres moich działań:
1. Przygotowanie oraz opisanie pomysłu na projekt

## Wkład w projekt:
Wybrałem temat oraz przygotowałem dokumentację

## Samoocena:
| Obszar | Ocena (1–5) | Komentarz |
|---------|--------------|-----------|
| Zaangażowanie | 5 | Zadanie wykonane w pełni |
| Wkład merytoryczny | 5 | Dokumentacja i wybór tematu |
| Komunikacja | 3 | Komunikacja na ostatnią chwilę |
| Terminowość | 3 | Zadanie zrealizowane na ostatnią chwilę |

## Refleksja:
Do poprawy komunikacja z zespołem oraz terminowość.
